...
def get_batch_cycle_date_vars(batch_cycle_date: Optional[str] = None, 
    ...
    if not batch_cycle_date:
        ...
        logger.info(f"Retrieved batch_cycle_date from Prefect variables: {batch_cycle_date}")
    ...
    if backfill_start_date and backfill_end_date:
        ...
        logger.info(f"Running in backfill mode from {backfill_start_date} to {backfill_end_date}.")
