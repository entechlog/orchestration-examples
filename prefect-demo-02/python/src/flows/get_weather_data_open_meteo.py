...
def get_weather_data(lat: str, lon: str, date: str) -> dict:
    ...
    if response.status_code == 200:
        ...
        logger.error(
            f"Failed to retrieve weather data: {response.status_code} - {response.text}"
        )
...
def get_location_data(lat: str, lon: str, date: str) -> dict:
    ...
    if response.status_code == 200:
        ...
        logger.error(
            f"Failed to retrieve location data: {response.status_code} - {response.text}"
        )
        response.raise_for_status()
    ...
    for date in dates:
        ...
        if lat and lon:
            ...
            location_data = get_location_data(
                lat=lat, lon=lon, date=date, wait_for=[weather_data]
            )
...
if __name__ == "__main__":
    get_weather_data_open_meteo(lat="40.7128", lon="-74.0060")
